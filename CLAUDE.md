...
**Provider switching:** Settings → Tasks offers Todoist, Things 3, and Markdown. Switching does NOT migrate tasks between providers — each backend remains its own canonical source. The selected provider follows the user through `task-provider.json`; keeping it separate from legacy `settings.json` prevents pre-Things builds from normalizing `things` back to `todoist`. The local schedule overlay is shared, so a chip dropped on Tuesday stays on Tuesday across a switch (though the id won't resolve to the new provider's tasks until the user moves it).
