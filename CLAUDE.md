...
## Auto-update — why we rolled our own

Custom GitHub-releases updater (`src/main/updater.ts`) because the app isn't
signed with an Apple Developer ID — `electron-updater`'s swap-and-relaunch
dies under Gatekeeper. Full mechanics, the `lsregister -f` + `open -n`
gotcha, the swap-log location, and the `bin/ycal`-not-in-dmg caveat:
...
`tasks.md` (cloudStore-routed) is a Todoist-parity task store: projects as
headings, `- [ ]` items with `@date` / `!priority` / `#duration` / `#energy`
/ `#location` tokens and an auto-assigned `^blockid`. Targeted line-level
edits only — never full-document rewrite. Full grammar + gotchas:
see `docs/markdown-task-format.md`.
